...
export default function AIClient({ initialLeads, initialError }: { initialLeads: LeadOption[]; initialError?: string }) {
  ...
  const run = async (key: string, action: () => Promise<any>, success?: string, failure = "This action could not be completed. Please try again.") => {
    ...
    } catch { setFeedback({ type: "error", text: failure }) }
    finally { setBusy("") }
    return null
  }

  const analyze = async () => {
    const result = await run("analyze", () => import("@/lib/actions/ai-assistant").then(({ analyzeLead }) => analyzeLead(selectedId)), "Lead analysis saved to the workspace.", "Unable to analyze this lead right now. Please try again.")
  ...
  return <div className="mx-auto max-w-7xl space-y-6">
    ...
    <section className="grid gap-6 xl:grid-cols-[0.72fr_1.28fr]">
      ...
      <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6"><div className="flex items-start justify-between gap-3"><div><div className="flex items-center gap-2 text-sm font-semibold text-slate-900"><Sparkles size={17} className="text-emerald-600" />Next best actions</div><p className="mt-1 text-xs text-slate-500">Every action uses this workspace&apos;s real lead and property data.</p></div><span className="rounded-full bg-slate-100 px-2.5 py-1 text-[11px] font-semibold text-slate-600">Server-side AI</span></div><div className="mt-5 grid gap-2 sm:grid-cols-2"><ActionButton onClick={analyze} busy={busy === "analyze"} icon={<Sparkles size={15} />}>Analyze Lead</ActionButton><ActionButton onClick={generateReply} busy={busy === "reply"} icon={<Mail size={15} />}>Generate Sales Reply</ActionButton><ActionButton onClick={findMatches} busy={busy === "matches"} icon={<Search size={15} />}>Find Matching Properties</ActionButton><ActionButton onClick={() => setShowFollowUp(true)} icon={<MessageSquareText size={15} />}>Create Follow-up</ActionButton><ActionButton onClick={() => setShowAppointment(true)} icon={<CalendarPlus size={15} />}>Schedule Appointment</ActionButton></div>{analysis && <div className="mt-6 rounded-xl border border-emerald-100 bg-emerald-50/60 p-4"><p className="text-sm font-semibold text-emerald-950">Qualification analysis</p><p className="mt-3 text-sm leading-6 text-slate-700">{analysis.leadSummary}</p><div className="mt-3 grid gap-3 sm:grid-cols-2"><Fact title="Intent" value={analysis.buyingSellingIntent || label(analysis.intent)} /><Fact title="Urgency" value={analysis.urgency || "Unclear"} /><Fact title="Property type" value={analysis.propertyType || "Not captured"} /><Fact title="Timeline" value={analysis.timeline || "Not captured"} /></div><AnalysisList title="Key requirements" items={analysis.keyRequirements} /><div className="mt-4 grid gap-3 sm:grid-cols-2"><AnalysisText title="Budget fit" value={analysis.budgetFit} /><AnalysisText title="Timeline assessment" value={analysis.timelineAssessment} /><AnalysisText title="Location / property fit" value={analysis.locationPropertyFit} /><AnalysisText title="Qualification summary" value={analysis.qualificationSummary} /></div><div className="mt-4 grid gap-3 sm:grid-cols-2"><AnalysisList title="Strengths" items={analysis.strengths} /><AnalysisList title="Risks / blockers" items={analysis.risksBlockers} /></div><p className="mt-4 text-sm font-semibold text-emerald-800">Next: {analysis.recommendedNextAction}</p><p className="mt-2 text-sm leading-6 text-slate-700"><span className="font-semibold text-slate-800">Sales strategy:</span> {analysis.salesStrategy}</p><p className="mt-2 text-xs text-slate-500"><span className="font-semibold">Priority reasoning:</span> {analysis.priorityReasoning?.join(" · ") || "Based on the captured lead requirements."}</p></div>}{reply && <div className="mt-6 rounded-xl border border-slate-200 bg-slate-50 p-4"><p className="text-sm font-semibold text-slate-900">Suggested sales reply</p><p className="mt-3 whitespace-pre-wrap text-sm leading-6 text-slate-700">{reply}</p></div>}</article>
    </section>
...
function AnalysisText({ title, value }: { title: string; value?: string }) { return <div className="rounded-lg bg-white/80 p-3"><p className="text-[10px] font-semibold uppercase tracking-wide text-slate-400">{title}</p><p className="mt-1 text-xs leading-5 text-slate-700">{value || "Not available"}</p></div> }
function AnalysisList({ title, items }: { title: string; items?: string[] }) { return <div className="mt-4 rounded-lg bg-white/80 p-3"><p className="text-[10px] font-semibold uppercase tracking-wide text-slate-400">{title}</p>{items?.length ? <ul className="mt-1 space-y-1 text-xs leading-5 text-slate-700">{items.map((item) => <li key={item}>• {item}</li>)}</ul> : <p className="mt-1 text-xs text-slate-500">None captured</p>}</div> }
